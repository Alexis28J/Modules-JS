...
function displayBooks(booksData) {
      ...
      for (const book of booksData.results) {
        ...
        topImg.src = book.formats["image/jpeg"];   //quando il nome è strano, è meglio metterlo con le quadre
...
function reduceSummary(summary){
    const maxLength = 20;
    let words = summary.split(' ');   //in questo modo posso dividere il testo senza il rischio di dover "tagliare" le parole 
    let end = '';
